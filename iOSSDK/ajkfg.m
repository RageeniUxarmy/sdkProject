...
@implementation ajkfg



+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ajkfg *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ajkfg *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (ajkfg *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (ajkfg *)subview;
        }
    }
    return nil;
}
...
- (void)hideAnimated:(BOOL)animated {
    ...
    if (self.mnjk > 0.0 && self.showStarted) {
        NSTimeInterval interv = [[NSDate date] timeIntervalSinceDate:self.showStarted];
        if (interv < self.mnjk) {
            NSTimer *timer = [NSTimer timerWithTimeInterval:(self.mnjk - interv) target:self selector:@selector(handleMinShowTimer:) userInfo:nil repeats:NO];
            [[NSRunLoop currentRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];
...
- (void)setupViews {
    ...
    backgroundView.sety = MBProgressHUDBackgroundStyleSolidColor;
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[roundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MBProgressHUDModeAnnularDeterminate) {
            [(roundProgressView *)indicator setDasf:YES];
        }
    } 
    ...
    if ([indicator respondsToSelector:@selector(setXProg:)]) {
...
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[ajkfg class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[roundProgressView class]]) {
        roundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [UXRoundProgressView appearanceWhenContainedIn:[MBProgressHUD class], nil];
#else
        appearance = [roundProgressView appearanceWhenContainedInInstancesOfClasses:@[[ajkfg class]]];
#endif
        if (appearance.progr == nil) {
            ((roundProgressView *)indicator).progr = color;
        }
        if (appearance.bgC == nil) {
            ((roundProgressView *)indicator).bgC = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MASG class]]) {
        MASG *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [UXBarProgressView appearanceWhenContainedIn:[MBProgressHUD class], nil];
#else
        appearance = [MASG appearanceWhenContainedInInstancesOfClasses:@[[ajkfg class]]];
#endif
        if (appearance.col == nil) {
            ((MASG *)indicator).col = color;
        }
        if (appearance.lsce == nil) {
            ((MASG *)indicator).lsce = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _xProg = 0.f;
        _dasf = NO;
        _progr = [[UIColor alloc] initWithWhite:1.f alpha:1.f];
        _bgC = [[UIColor alloc] initWithWhite:1.f alpha:.1f];
    }
    return self;
}
...
- (void)setProgr:(UIColor *)progressTintColor {
    NSAssert(progressTintColor, @"The color should not be nil.");
    if (progressTintColor != _progr && ![progressTintColor isEqual:_progr]) {
        _progr = progressTintColor;
        [self setNeedsDisplay];
    }
}

- (void)setBgC:(UIColor *)backgroundTintColor {
    NSAssert(backgroundTintColor, @"The color should not be nil.");
    if (backgroundTintColor != _bgC && ![backgroundTintColor isEqual:_bgC]) {
        _bgC = backgroundTintColor;
        [self setNeedsDisplay];
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (_dasf) {
        ...
        endAngle = (self.xProg * 2 * (float)M_PI) + startAngle;
        ...
        if (isPreiOS7) {
            ...
            CGFloat endAngle = (self.xProg * 2.f * (float)M_PI) + startAngle;
            [_progr setFill];
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        _sdf = 0.f;
        _lsce = [UIColor whiteColor];
        _col = [UIColor whiteColor];
        _eeesa = [UIColor clearColor];
        self.backgroundColor = [UIColor clearColor];
        self.opaque = NO;
    }
    return self;
...
- (void)setCol:(UIColor *)progressColor {
    NSAssert(progressColor, @"The color should not be nil.");
    if (progressColor != _col && ![progressColor isEqual:_col]) {
        _col = progressColor;
        [self setNeedsDisplay];
    }
}

- (void)setEeesa:(UIColor *)progressRemainingColor {
    NSAssert(progressRemainingColor, @"The color should not be nil.");
    if (progressRemainingColor != _eeesa && ![progressRemainingColor isEqual:_eeesa]) {
        _eeesa = progressRemainingColor;
        [self setNeedsDisplay];
    }
}
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context,[_lsce CGColor]);
    CGContextSetFillColorWithColor(context, [_eeesa CGColor]);
    ...
    CGContextSetFillColorWithColor(context, [_col CGColor]);
    radius = radius - 2;
    CGFloat amount = self.sdf * rect.size.width;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _sety = MBProgressHUDBackgroundStyleBlur;
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
                _olcs = [UIColor colorWithWhite:0.8f alpha:0.6f];
            } else {
                _olcs = [UIColor colorWithWhite:0.95f alpha:0.6f];
            }
        } else {
            _sety = MBProgressHUDBackgroundStyleSolidColor;
            _olcs = [[UIColor blackColor] colorWithAlphaComponent:0.8];
        }
...
- (void)setSety:(MBProgressHUDBackgroundStyle)style {
...
- (void)setOlcs:(UIColor *)color {
    NSAssert(color, @"The color should not be nil.");
    if (color != _olcs && ![color isEqual:_olcs]) {
        _olcs = color;
...
- (void)updateForBackgroundStyle {
    MBProgressHUDBackgroundStyle style = self.sety;
    if (style == MBProgressHUDBackgroundStyleBlur) {
        ...
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            ...
            self.backgroundColor = self.olcs;
            self.layer.allowsGroupOpacity = NO;
            ...
            toolbar.barTintColor = self.olcs;
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.sety == MBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.olcs;
        } else {
#if !TARGET_OS_TV
...
@implementation ajkfg (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [ajkfg allHUDsForView:view];
    for (ajkfg *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (UIColor *)color {
    return self.bezelView.olcs;
}

- (void)setColor:(UIColor *)color {
    MBMainThreadAssert();
    self.bezelView.olcs = color;
}
...
- (BOOL)dimBackground {
    ...
    return backgroundView.sety == MBProgressHUDBackgroundStyleSolidColor && [backgroundView.olcs isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.sety = MBProgressHUDBackgroundStyleSolidColor;
    self.backgroundView.olcs = dimBackground ? [UIColor colorWithWhite:0.f alpha:.2f] : [UIColor clearColor];
}
